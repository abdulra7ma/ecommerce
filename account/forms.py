...
class UserEditForm(forms.ModelForm):
    ...
    def clean_user_name(self):
        usr_name = self.cleaned_data["name"]
        u = Customer.objects.filter(name=usr_name)
        if u.exists():
            raise forms.ValidationError(
                "This user name already exists please use another name"
            )
        return usr_name
...
class UserReactivateForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not user.exists():
            raise forms.ValidationError(
                "Sorry, this email does not exists. Please try another email."
            )
        elif user.exists() and user[0].is_active is True:
